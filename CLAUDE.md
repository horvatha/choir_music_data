...
The breakage isn't always a hard error, either — a loader change here can silently change what `concert_music_app` *renders* without touching its schema at all: the resulting rows are still perfectly valid, just wrong (found 2026-08-23, a `load_birth_death_places.py` fix flattened Leningrad/Petrograd into "Saint Petersburg" for every era — nothing errored, the bug only showed up by actually loading the rendered page). Before committing a change to a place/nationality/date loader (`load_birth_death_places.py`, `load_nationalities.py`, `load_place_period_names.py`, `load_place_hierarchy.py`), run `concert_music_app`'s regression suite against your local dev DB: `cd ../concert_music_app && uv run pytest tests/` (design rationale in that repo's `TESTING_PLAN.md`) — it hits real rendered pages for known tricky cases (a landmark's containing settlement, a dual-tagged nationality's translation, a place with real per-era renames) that a loader-only check can't see.
