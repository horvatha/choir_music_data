...
match_polmic_composers.py's composers_polmic_matched.csv, in_the_db=true
rows) into other_webpages, linking each to its composer row -- same
purpose and shape as load_swedish_heritage.py, matching primarily by
wikidata_id (already resolved by the matching script) since that's
unambiguous, falling back to adapters/name_matching's normalize_tokens()
only for the rare row where the matched DB composer has no wikidata_id of
its own yet.

No new composer rows are created here, and no dates/bio/works content is
loaded into the DB from POLMIC yet -- this is just the article link, the
same first step LFZE/Swedish Heritage went through before any of their
other data got used.
...
MATCHED_CSV = Path(__file__).resolve().parent / "data" / "POLMIC" / "composers_polmic_matched.csv"
JSON_DIR = Path(__file__).resolve().parent / "data" / "POLMIC"
SOURCE = "polmic"
        ...
        with conn.cursor() as cur:
            ...
            for row in rows:
                ...
                cur.execute(INSERT_OTHER_WEBPAGE_SQL, {
                    ...
                    "language": "en", "title": data["name"], "author": None, "fetched_at": fetched_at,
                })
